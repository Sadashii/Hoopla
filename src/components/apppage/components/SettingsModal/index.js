...
const SettingsModal = ({
  ...
  const [currentPanel, setCurrentPanel] = useStickyState("my_account", "user_current_settings_panel")
  
  const account_settings = {
    "my_account": <FlexBox
      fullWidth align
      className={clsx(currentPanel === "my_account" && styles.settingNavSelected, styles.settingNav)}
  ...
  const workspace_settings = {
    "ws_settings": <FlexBox
      fullWidth align
      className={clsx(currentPanel === "ws_settings" && styles.settingNavSelected, styles.settingNav)}
    ...
    "ws_members": <FlexBox
      fullWidth align
      className={clsx(currentPanel === "ws_members" && styles.settingNavSelected, styles.settingNav)}
  ...
  const UpdateCancelFooter = ({onUpdate, onCancel}) => {
    return (
      <FlexBox align className={styles.updateOrCancelFooter}>
        <Button variant={"contained"} className={styles.buttonUpdate} onClick={onUpdate}>Update</Button>
        <Button variant={"outlined"} className={styles.buttonCancel} onClick={onCancel}>Cancel</Button>
      </FlexBox>
    )
  }
    ...
    const onUpdate = () => {
      if (modifiedData.username && modifiedData.username !== user.username) {
        axios
          ...
          .catch(err => {
            alert.show("Unable to update account. Please try again later.", {type: types.ERROR})
          })
      }
      ...
      if (modifiedData.password || modifiedData.repeatPassword) {
        ...
        if (!cancelled && modifiedData.password !== modifiedData.repeatPassword) {
          setField("passwordError")("The entered password's do not match")
          cancelled = true
        }
        if (!cancelled && modifiedData.password.length < 8) {
          setField("passwordError")("The password needs to be atleast 8 characters")
          cancelled = true
        }
        if (!cancelled) {
          ...
          axios
            ...
            .catch(err => {
              alert.show("Unable to update password. Please try again later.", {type: types.ERROR})
            })
        }
      }
    }
    ...
    return (
      <>
        <FlexBox fullwidth justifyBetween>
          ...
          <IconDisplayAndUploader size={'5rem'} currentSrc={user.meta.profilePicture} onChange={() => {}} />
        </FlexBox>
        ...
        <FlexBox column fullWidth className={styles.settingCategory}>
          ...
          <TextField
            variant="outlined"
            fullWidth
            value={modifiedData.username === undefined ? user.username : modifiedData.username}
        ...
        <FlexBox column fullWidth className={styles.settingCategory}>
          ...
          {modifiedData.passwordError && (
            <span className={styles.inputLabel} style={{color: "red"}}>{modifiedData.passwordError}</span>
          )}
          <span className={styles.inputLabel}>Remember to use a variety of characters and digits for a safe password</span>
        </FlexBox>
  ...
  return (
    <ClosePopup open={open} onClose={onClose} className={styles.settingsModal} defaultClass>
